...
/// Test helper that wraps init with testing io/environ for the 2-arg call pattern.
pub fn testInit(alloc: Allocator, opts: Options) !Terminal {
    return init(alloc, std.testing.io, &test_env, opts);
}
...
test "Terminal: setCursorPos saturates overflowing origin offsets" {
    const alloc = testing.allocator;
    var t = try testInit(alloc, .{ .cols = 10, .rows = 10 });
    defer t.deinit(alloc);
...
test "Terminal: input with no control characters" {
    const alloc = testing.allocator;
    var t = try testInit(alloc, .{ .cols = 40, .rows = 40 });
    defer t.deinit(alloc);
...
test "Terminal: input with basic wraparound" {
    const alloc = testing.allocator;
    var t = try testInit(alloc, .{ .cols = 5, .rows = 40 });
    defer t.deinit(alloc);
...
test "Terminal: input that forces scroll" {
    const alloc = testing.allocator;
    var t = try testInit(alloc, .{ .cols = 1, .rows = 5 });
    defer t.deinit(alloc);
...
test "Terminal: input unique style per cell" {
    const alloc = testing.allocator;
    var t = try testInit(alloc, .{ .cols = 30, .rows = 30 });
    defer t.deinit(alloc);
...
test "Terminal: zero-width character at start" {
    var t = try testInit(testing.allocator, .{ .cols = 80, .rows = 80 });
    defer t.deinit(testing.allocator);
...
test "Terminal: print single very long line" {
    var t = try testInit(testing.allocator, .{ .rows = 5, .cols = 5 });
    defer t.deinit(testing.allocator);
...
test "Terminal: print wide char with 1-column width" {
    const alloc = testing.allocator;
    var t = try testInit(alloc, .{ .cols = 1, .rows = 2 });
    defer t.deinit(alloc);
...
test "Terminal: print wide char in single-width terminal" {
    var t = try testInit(testing.allocator, .{ .cols = 1, .rows = 80 });
    defer t.deinit(testing.allocator);
...
test "Terminal: variation selectors apply to preceding codepoint" {
    var t = try testInit(testing.allocator, .{ .cols = 5, .rows = 5 });
    defer t.deinit(testing.allocator);
...
test "Terminal: VS15 to make narrow character with pending wrap" {
    var t = try testInit(testing.allocator, .{ .rows = 5, .cols = 4 });
    defer t.deinit(testing.allocator);
...
test "Terminal: VS16 to make wide character on next line" {
    var t = try testInit(testing.allocator, .{ .rows = 5, .cols = 3 });
    defer t.deinit(testing.allocator);
...
test "Terminal: print Devanagari grapheme should be wide on next page" {
    ...
    var t = try testInit(testing.allocator, .{ .rows = rows, .cols = cols });
    defer t.deinit(testing.allocator);
...
test "Terminal: print writes to bottom if scrolled" {
    var t = try testInit(testing.allocator, .{ .cols = 5, .rows = 2 });
    defer t.deinit(testing.allocator);
...
test "Terminal: soft wrap" {
    var t = try testInit(testing.allocator, .{ .cols = 3, .rows = 80 });
    defer t.deinit(testing.allocator);
...
test "Terminal: print right margin wrap" {
    var t = try testInit(testing.allocator, .{ .cols = 10, .rows = 5 });
    defer t.deinit(testing.allocator);
...
test "Terminal: linefeed unsets pending wrap" {
    var t = try testInit(testing.allocator, .{ .cols = 5, .rows = 80 });
    defer t.deinit(testing.allocator);
...
test "Terminal: horizontal tabs" {
    const alloc = testing.allocator;
    var t = try testInit(alloc, .{ .cols = 20, .rows = 5 });
    defer t.deinit(alloc);
...
test "Terminal: insertLines handles style refs" {
    const alloc = testing.allocator;
    var t = try testInit(alloc, .{ .cols = 5, .rows = 3 });
    defer t.deinit(alloc);
...
test "Terminal: insertLines across page boundary marks all shifted rows dirty" {
    const alloc = testing.allocator;
    var t = try testInit(alloc, .{ .rows = 5, .cols = 10, .max_scrollback = 1024 });
    defer t.deinit(alloc);
...
test "Terminal: insertLines (legacy test)" {
    const alloc = testing.allocator;
    var t = try testInit(alloc, .{ .cols = 2, .rows = 5 });
    defer t.deinit(alloc);
...
test "Terminal: insertLines with scroll region" {
    const alloc = testing.allocator;
    var t = try testInit(alloc, .{ .cols = 2, .rows = 6 });
    defer t.deinit(alloc);
...
test "Terminal: insertLines resets wrap" {
    const alloc = testing.allocator;
    var t = try testInit(alloc, .{ .rows = 3, .cols = 3 });
    defer t.deinit(alloc);
...
test "Terminal: scrollUp creates scrollback in primary screen" {
    ...
    const alloc = testing.allocator;
    var t = try testInit(alloc, .{ .rows = 5, .cols = 5, .max_scrollback = 10 });
    defer t.deinit(alloc);
...
test "Terminal: scrollUp with max_scrollback zero and left/right margin" {
    ...
    const alloc = testing.allocator;
    var t = try testInit(alloc, .{ .rows = 5, .cols = 10, .max_scrollback = 0 });
    defer t.deinit(alloc);
...
test "Terminal: scrollDown preserves pending wrap" {
    const alloc = testing.allocator;
    var t = try testInit(alloc, .{ .cols = 5, .rows = 10 });
    defer t.deinit(alloc);
...
test "Terminal: eraseChars wide char boundary conditions" {
    const alloc = testing.allocator;
    var t = try testInit(alloc, .{ .rows = 1, .cols = 8 });
    defer t.deinit(alloc);
...
test "Terminal: eraseChars wide char splits proper cell boundaries" {
    const alloc = testing.allocator;
    var t = try testInit(alloc, .{ .rows = 1, .cols = 30 });
    defer t.deinit(alloc);
...
test "Terminal: eraseChars wide char wrap boundary conditions" {
    const alloc = testing.allocator;
    var t = try testInit(alloc, .{ .rows = 3, .cols = 8 });
    defer t.deinit(alloc);
...
test "Terminal: reverseIndex top of scrolling region" {
    const alloc = testing.allocator;
    var t = try testInit(alloc, .{ .cols = 2, .rows = 10 });
    defer t.deinit(alloc);
...
test "Terminal: deleteLines (legacy)" {
    const alloc = testing.allocator;
    var t = try testInit(alloc, .{ .cols = 80, .rows = 80 });
    defer t.deinit(alloc);
...
test "Terminal: DECALN" {
    const alloc = testing.allocator;
    var t = try testInit(alloc, .{ .cols = 2, .rows = 2 });
    defer t.deinit(alloc);
...
test "Terminal: decaln reset margins" {
    const alloc = testing.allocator;
    var t = try testInit(alloc, .{ .cols = 3, .rows = 3 });
    defer t.deinit(alloc);
...
test "Terminal: insertBlanks pushes off end" {
    ...
    // tests might actually be verifying wrong behavior.
    const alloc = testing.allocator;
    var t = try testInit(alloc, .{ .cols = 3, .rows = 2 });
    defer t.deinit(alloc);
...
test "Terminal: insertBlanks outside left/right scroll region" {
    const alloc = testing.allocator;
    var t = try testInit(alloc, .{ .cols = 6, .rows = 10 });
    defer t.deinit(alloc);
...
test "Terminal: insertBlanks shifts hyperlinks" {
    ...
    const alloc = testing.allocator;
    var t = try testInit(alloc, .{ .cols = 10, .rows = 2 });
    defer t.deinit(alloc);
...
test "Terminal: tabClear single" {
    const alloc = testing.allocator;
    var t = try testInit(alloc, .{ .cols = 30, .rows = 5 });
    defer t.deinit(alloc);
...
test "Terminal: resize with left and right margin set" {
    ...
    const cols = 70;
    const rows = 23;
    var t = try testInit(alloc, .{ .cols = cols, .rows = rows });
    defer t.deinit(alloc);
