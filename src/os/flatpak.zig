...
pub const FlatpakHostCommand = struct {
    ...
    pub fn signal(self: *FlatpakHostCommand, sig: u8, pg: bool) !void {
        ...
        var g_err: ?*glib.Error = null;
        defer if (g_err) |ptr| ptr.free();
        const bus = gio.busGetSync(.session, null, &g_err) orelse {
            log.warn("signal error getting bus: {s}", .{g_err.?.f_message.?});
        ...
        const reply = bus.callSync(
            ...
            glib.VariantType.checked("()"),
            .{},
            std.math.maxInt(c_int),
            null,
            &g_err,
        ) orelse {
            log.warn("signal send error: {s}", .{g_err.?.f_message.?});
    ...
    fn threadMain(self: *FlatpakHostCommand, alloc: Allocator) void {
        ...
        const loop = glib.MainLoop.new(ctx, 1);
        defer loop.unref();
        ...
        defer if (g_err) |ptr| ptr.free();
        const bus = gio.busGetSync(.session, null, &g_err) orelse {
            log.warn("spawn error getting bus: {s}", .{g_err.?.f_message.?});
    ...
    fn start(
        ...
        defer fd_list.unref();
        if (fd_list.append(self.stdin, &err) < 0) {
            log.warn("error adding fd: {s}", .{err.?.f_message.?});
            return Error.FlatpakSetupFail;
        }
        if (fd_list.append(self.stdout, &err) < 0) {
            log.warn("error adding fd: {s}", .{err.?.f_message.?});
            return Error.FlatpakSetupFail;
        }
        if (fd_list.append(self.stderr, &err) < 0) {
            log.warn("error adding fd: {s}", .{err.?.f_message.?});
            return Error.FlatpakSetupFail;
        }

        // Build our arguments for the file descriptors.
        const fd_builder = glib.VariantBuilder.new(glib.VariantType.checked("a{uh}"));
        defer fd_builder.unref();
        fd_builder.add("{uh}", @as(c_int, 0), self.stdin);
        fd_builder.add("{uh}", @as(c_int, 1), self.stdout);
        fd_builder.add("{uh}", @as(c_int, 2), self.stderr);

        // Build our env vars
        const env_builder = glib.VariantBuilder.new(glib.VariantType.checked("a{ss}"));
        defer env_builder.unref();
        if (self.env) |env| {
            var it = env.iterator();
            while (it.next()) |pair| {
                const key = try arena.dupeZ(u8, pair.key_ptr.*);
                const value = try arena.dupeZ(u8, pair.value_ptr.*);
                env_builder.add("{ss}", key.ptr, value.ptr);
            }
        }
        ...
        // common code path.
        const g_cwd = glib.getCurrentDir();
        defer glib.free(g_cwd);
        ...
        _ = params.refSink(); // take ownership
        defer params.unref();

        // Subscribe to exit notifications
        const subscription_id = bus.signalSubscribe(
            "org.freedesktop.Flatpak",
            "org.freedesktop.Flatpak.Development",
            "HostCommandExited",
            "/org/freedesktop/Flatpak/Development",
        ...
        errdefer bus.signalUnsubscribe(subscription_id);

        // Go!
        const reply = bus.callWithUnixFdListSync(
            "org.freedesktop.Flatpak",
            "/org/freedesktop/Flatpak/Development",
            "org.freedesktop.Flatpak.Development",
            "HostCommand",
            params,
            glib.VariantType.checked("(u)"),
            .{},
            std.math.maxInt(c_int),
            fd_list,
            null,
            ...
            log.warn("Flatpak.HostCommand failed: {s}", .{err.?.f_message.?});
            return Error.FlatpakRPCFail;
        };
        defer reply.unref();
    ...
    fn onExit(
        ...
        params.get("(uu)", &pid, &exit_status_raw);
        ...
        bus.signalUnsubscribe(state.subscription);
